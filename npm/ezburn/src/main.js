...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.1.0") {
        throw new Error(`Cannot start service: Host version "${"0.1.0"}" does not match binary version ${quote(binaryVersion)}`);
      }
      return;
    }
...
var EZBURN_BINARY_PATH = process.env.EZBURN_BINARY_PATH || EZBURN_BINARY_PATH;
var isValidBinaryPath = (x) => !!x && x !== "/usr/bin/ezburn";
var packageDarwin_arm64 = "@ezburn/darwin-arm64";
var packageDarwin_x64 = "@ezburn/darwin-x64";
  ...
  if (/\.zip\//.test(binPath)) {
    ...
    if (pnpapi) {
      ...
      const binTargetPath = path.join(
        ...
        ".cache",
        "ezburn",
        `pnpapi-${pkg.replace("/", "-")}-${"0.1.0"}-${path.basename(subpath)}`
      );
...
var _a;
var isInternalWorkerThread = ((_a = worker_threads == null ? void 0 : worker_threads.workerData) == null ? void 0 : _a.ezburnVersion) === "0.1.0";
var ezburnCommandAndArgs = () => {
  if ((!EZBURN_BINARY_PATH || false) && (path2.basename(__filename) !== "main.js" || path2.basename(__dirname) !== "lib")) {
    throw new Error(
      `The ezburn JavaScript API cannot be bundled. Please mark the "ezburn" package as external so it's not included in the bundle.

More information: The file containing the code for ezburn's JavaScript API (${__filename}) does not appear to be inside the ezburn package on the file system, which usually means that the ezburn package was bundled into another file. This is problematic because the API needs to run a binary executable inside the ezburn package which is located using a relative path from the API code to the executable. If the ezburn package is bundled, the relative path will be incorrect and the executable won't be found.`
    );
  }
  if (false) {
    return ["node", [path2.join(__dirname, "..", "bin", "ezburn")]];
  } else {
...
var ensureServiceIsRunning = () => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.1.0"}`, "--ping"), {
    windowsHide: true,
...
var runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.1.0"}`), {
    cwd: defaultWD,
    windowsHide: true,
    input: stdin,
...
var randomFileName = () => {
  return path2.join(os2.tmpdir(), `ezburn-${crypto.randomBytes(32).toString("hex")}`);
};
var workerThreadService = null;
var startWorkerThreadService = (worker_threads2) => {
  let { port1: mainPort, port2: workerPort } = new worker_threads2.MessageChannel();
  let worker = new worker_threads2.Worker(__filename, {
    workerData: { workerPort, defaultWD, ezburnVersion: "0.1.0" },
